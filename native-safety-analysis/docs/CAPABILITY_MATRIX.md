...
| compilation_reuse | implemented | **verified**（9 项恒等测试：缓存命中 == 清缓存全新求解，逐字段含 undefined reasons；相位基准入库 `run_incremental_bench.py`：compile 31–45% 且与概率无关；n=1500 提速 1.53–1.62×） | 未执行 | tests/test_compile_cache.py, verification/run_incremental_bench.py | 结构指纹 = 事件 ID + 门（含输入序）+ 顶事件；**不含概率**；table/importance 每次全量重算，正确性不依赖缓存 |
...
| object_level_patch_language | implemented | **verified**（55 项专项测试 + 状态机契约：十种操作按序应用、纯函数、**与等价全模型提案同哈希**（收敛，含 rate 编辑与 rate 新增）、rate p 不可直设、**`set_event_rate`/`add_event(rate)` 完整重跑转换闸门且 q 对照独立 oracle（1e-30）**、**首个 rate 事件自动升 schema 0.2.0 而语义声明必须显式（`set_probability_semantics`）**、schema 只升不降、被引用不可删、跨操作环与不可达环被拒、非法记 invalid、`1/3` 精确保留） | 未执行 | tests/test_patch.py, verification/run_store_verification.py | `model-patch-v1`：作用于库内当前基线的规范形式，产出普通提案；`set_event_rate` 仅编辑既有 rate 事件、`add_event` 可新增 rate 事件（均不提供普通→rate 转换）；schema 只升不降；无移动/重命名 |
...
- 可声明：对提案给出**结构化影响范围分析**（事件/门/假设/顶事件的增删改，两侧均取自库内行），提供**一等公民 revert**（从库内基线自建提案，仍走两步人类批准，stale 双向重派生）与**对象级 patch 语言**（十种编辑操作，作用于库内规范形式，与整份模型提案收敛于同一哈希；rate 编辑/新增重跑转换闸门并对照独立 oracle）
