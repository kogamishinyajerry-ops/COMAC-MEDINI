...
- **patch 是纯函数**：`apply_patch(canonical, ops)` 深拷贝后按序应用八种编辑操作，输入永不改写；终态整体校验（引用/全图无环/K_OF_N/双向语义守卫）后才返回。
...
- **守卫**：rate 事件的 p 不可直设（由 λ·t 派生）；被引用对象不可删；跨操作构造的环被终检拒绝；非法 patch 记为 `invalid`（`PATCH_OP`）。
