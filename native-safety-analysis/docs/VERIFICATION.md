...
**rate 编辑（`set_event_rate`）不变量：**

56. `set_event_rate` 重派生 p 与完整 provenance（λ/t/λt/精度），且 **q 对照独立有理级数 oracle 在声明容差 1e-30 内**（与 `run_rate_cross_check` 同口径；oracle 与生产的舍入定义不同，靠容差对照——这正是既有约定）
57. **完整重跑转换闸门**且**保留原始错误码**：负 λ → `RATE_VALUE`、非法单位 → `RATE_UNITS`、repairable/weibull/禁字段 → `RATE_UNSUPPORTED`、空 source → `SOURCE`（不拍平成 `PATCH_OP`——分类信息不丢失）
