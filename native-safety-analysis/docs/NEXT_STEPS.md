...
- **`add_event(rate)` + `set_probability_semantics`：新增 rate 事件（B07 后半之三）** ✅
  - `add_event` 接受 `p` **或** `rate` 规格；新增走与 `set_event_rate` **同一转换闸门**（错误码保留底层分类），q 对照独立 oracle（1e-30），与等价混合模型**收敛同哈希**
  - **语义声明是显式操作**：`set_probability_semantics`（与 `set_condition` 同级）——假设不被编辑的副作用翻转，作者必须声明；双向守卫兜底
  - **schema 只升不降**：首个 rate 事件自动 0.1.0→0.2.0；删光 rate 事件后版本与语义保持，终检拒绝（降级须显式决定）
  - 新增精度取模型口径（有 rate 兄弟沿用其记录精度，否则域默认 40 位）
