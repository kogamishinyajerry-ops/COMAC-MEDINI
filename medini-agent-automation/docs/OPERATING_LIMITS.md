...
- 单写者：跨进程文件锁；每次持锁带唯一 token，只释放自己的锁
- 恢复记录：落盘中断写 `recovery.jsonl`，由 `read_project.pending_recovery` 暴露
- CLI：`key-init` / `approve` / `trust`

### 写锁行为契约（2026-09-22 起生效 —— 行为变化，操作规程须同步）

**锁龄不构成抢占授权。** 旧版按 `stale_after_s` 自动删除"看起来太旧"的锁——
但工程作业可以运行很久，锁龄超阈值不能证明持锁进程已停止；自动抢占等于允许
两个写者同时进入同一作用域。现行契约 fail closed：

- 已存在 / 老旧 / 损坏 / 无法解析的锁一律拒绝写入（`WRITER_BUSY`），**不自动抢占**
- `stale_after_s` 仅为兼容保留的元数据，不授予任何权限
- 等待用单调时钟；拒绝负数与非有限等待参数
- 写入失败时保留（可能不完整的）锁记录，绝不把"不确定"当成"未上锁"
- **宁可暂停作业等待受控恢复，也不自动夺锁** —— 依赖"15 分钟自动解锁"的
  调用方必须修改操作规程

**锁恢复规程**（遇到 `WRITER_BUSY` 且确认原写者已不在时）：

1. 暂停所有会发起写作业的入口（Agent / CLI / 定时任务）
2. 确认相关写进程与 medini 工具作业**全部停止**（任务管理器核对，不留侥幸）
3. 回读工程副本内容与作业记录，评估是否有半成品写入
4. 归档锁文件（`locks/<slug>.lock` 移走留证，不直接删）
5. 由受控管理员解除后再放行入口；重跑只在新工作副本上进行

**写者仍可能活动时，禁止人工删除或替换锁文件。**
...
- **写锁面向可信本地文件系统上的合作进程**：不覆盖网络文件系统、恶意删锁、
  分布式 fencing；无人值守的强制解锁接口是有意不提供的
- 未测崩溃恢复端到端（没有真的 kill 一次写到一半的进程）
- 幂等记录含首次完整结果，随变更数线性增长，目前无自动清理
- `wait_s` 默认 0（立即失败，不排队）—— 写作业是秒级的，直接告知
  "有别的写者"比把 Agent 挂住更有用
